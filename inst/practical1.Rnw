...
Installing the the course R package is straightforward. First install \cc{drat}:
...
\begin{itemize}
\item What happens if you try to pass a value for \cc{N} or \cc{p} that violates the constraints in the \cc{data} block? For example, what happens if you set:
...
\item Add a \cc{transformed parameter} block and include a print statement to check whether the constants, \cc{N} and \cc{p}, have been correctly passed.
...
<<eval=echo, echo=echo, messages=FALSE, warning=FALSE>>=
