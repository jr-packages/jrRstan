...
generated quantities {
  real dev; // Deviance
  dev = deviance(y, x, subj_label, alpha0, alpha1, beta0, sigsq);
}
"
stanhlv2(m)
@
...
generated quantities {
  real dev; // Deviance
  dev = deviance(y, x, subj_label, alpha0, alpha1, beta0, 
                 beta1, sigsq);
}
"
stanhlv2(m)
...
generated quantities {
  vector<lower=0, upper=1>[N] p; /* Study-specific probability
                                    of tumour */ 
  real<lower=0, upper=1> p_pop; /* \"Population\" probability
                                    of tumour */
  p = inv_logit(theta);
  p_pop = inv_logit(mu); 
}
